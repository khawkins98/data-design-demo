 ...
 *
 * `packages/undrr-tokens` scopes its custom properties to a `.undrr-tokens`
 * class rather than `:root`, deliberately: tokens at `:root` would theme the
 * host's canary elements and defeat the leakage assertion.
 *
 ...
 *                                                          default of 100000
 *
 * A transparent, borderless calendar rendered over the page content.
 *
 * On the Delta host it is worse than on Mangrove, because Tailwind Preflight's
 * `*{border:0 solid;padding:0;margin:0}` applies inside the portal too. Without
 * the token scope class the overlay has no background, no border AND no
 * padding, so it collapses to the size of its text with the page showing
 * through. Verified in the e2e run.
 *
 ...
 * lines, but you have to know to do it, and nothing warns you: a failed
 * `var()` is silent.
 *
...
/** Tooltips are portalled too, and render with no background without this. */
