...
| **Design-token alignment** - can it be driven by UNDRR tokens, and does a token change propagate? | [A5 Visual control and theming fidelity](./decision-axes.md#a5---visual-control-and-theming-fidelity), with [A2](./decision-axes.md#a2---estate-change-amplification) | `theming.*` in each `evidence.json`, `theming-control.json`, `change-amplification.json` |
